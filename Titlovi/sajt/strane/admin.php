...
<head>
	...
	<meta name="description" content="Admin panel, za dodavanje pisanje i pregledanje korisnika tiitlova i menija" />
	<meta name="keywords" content="admin, meni, srpskom, titlovi" />
...
?>
	<div id="omot">
		...
		<div id="sadrzaj">
			<h2>Admin panel</h2>
			<div id="admin">
			<form action="<?php print $_SERVER['PHP_SELF'];?>" method="POST" name="admin">
				Izaberite tabelu: 
				<select id="lista"name="ddlTable">
					<option value="izaberite">Izaberite</option>
					<option value="meni">Meni</option>
					<option value="korisnici">Korisnici</option>
					<option value="titlovi">Titlovi</option>
				</select>
				<input type="submit" name="btnIzaberi" value="Potvrdite" class="izaberite">
				<div class="cisti"></div>
				<?php
					switch(@$_REQUEST['ddlTable']){
						case 'meni':
							include("inc/meni.inc");
							break;
						case 'korisnici';
							include("inc/korisnici.inc");
							break;
						case 'titlovi':
							include("inc/titlovi.inc");
							break;
						default:
							if(isset($_REQUEST['btnIzaberi'])){
							echo "<div id='ispis'>Niste odabrali nista za izmenu</div>";
							}
							else{
								echo "<div id='ispis4'>Odaberite stavku iz liste</div>";
							}
					}
				?>
			</form>
